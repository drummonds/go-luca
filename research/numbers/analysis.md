...
### pglike NUMERIC → SQLite translation (CRITICAL)
...
**Fix needed**: Translate `NUMERIC(p,s)` to `TEXT` instead of `REAL`. All Go decimal libraries round-trip losslessly through text strings. This matches how pglike already handles `TIMESTAMP` → `TEXT`.
